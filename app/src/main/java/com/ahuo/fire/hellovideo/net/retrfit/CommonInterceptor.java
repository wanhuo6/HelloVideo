package com.ahuo.fire.hellovideo.net.retrfit;
...
public class CommonInterceptor implements Interceptor {
    ...
    private void printRequest(Request request) throws UnsupportedEncodingException {
        MLog.e("request method:" + request.method());

        //打印GET请求参数
        Set<String> set = request.url().queryParameterNames();
        for (String s : set) {
            MLog.e("request key:" + s
        ...
        if (request.body() instanceof FormBody) {
            ...
            for (int i = 0; i < oldFormBody.size(); i++) {
                MLog.e("request key:" + URLDecoder.decode(oldFormBody.encodedName(i), "utf-8")
